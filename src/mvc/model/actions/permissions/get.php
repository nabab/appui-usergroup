...
if ( !empty($model->data['id']) && (!empty($model->data['id_user']) || !empty($model->data['id_group'])) ){
  ...
  $id_group = !empty($model->data['id_user']) ?
    $model->db->selectOne('bbn_users', 'id_group', ['id' => $model->data['id_user']]) :
    $model->data['id_group'];
  foreach ( $model->inc->options->options($model->data['id']) as $id_opt => $opt ){
    if ( !empty($model->data['id_user']) ){
      if ( !empty($model->inc->pref->userHas($id_opt, $model->data['id_user'])) ){
        $res['user'][] = $id_opt;
      }
    }
    if ( !empty($model->inc->pref->groupHas($id_opt, $id_group)) ){
      $res['group'][] = $id_opt;
    }
